...
export interface SelectFileProps {
  ...
  receiveValidatonResults: (
    callback: (event: Electron.IpcMainEvent, args: Array<any>) => void
  ) => void
...
const SelectFile: React.FC<SelectFileProps> = (props) => {
  ...
  const selectFilePath = () => {
    window.fileSystemApi.receiveSelectedFilePath(
      (event: Electron.IpcMainEvent, path: string) => {
        ...
        props.receiveValidatonResults(
          (event: Electron.IpcMainEvent, result: Array<any>) => {
            ...
            if (isValid) {
              ...
              console.error(
                `Could not validate selected file as a : ${props.fileName} file.`
              )
  ...
  return (
    <>
      <VStack mt="20px" mb="20px" spacing={1}>
        {!props.filePath && (
          <>
            <Center>
        ...
        {fileValidationError && (
          <Center>
            ...
            <Text fontSize="12px" ml="5px" color="red.warning">
              Invalid {props.fileName}.json file: Please select a valid{" "}
              {props.fileName}.json file to continue.
            </Text>
          </Center>
        )}
