...
ipcMain.on("req-gen-val-keys-and-encrypt",  async (event, args) => {
    var [mnemonic, password, stakeInfo, chain, address, mnemonicOption, importPassword] = args
    try {
        const savePath = await genValidatorKeysAndEncrypt(event, mnemonic, password, stakeInfo, chain, address, mnemonicOption, importPassword)
